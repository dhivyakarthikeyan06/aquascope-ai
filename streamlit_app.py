...
st.subheader(
    "AI-Powered Aquatic Microorganism Screening"
)
    ...
    if uploaded_file:
        ...
        if st.button("🔍 Analyze Image"):
            ...
            if is_unknown:
                ...
                st.write(
                    f"Similarity to known classes: "
                    f"{best_similarity:.3f}"
                )

                st.warning(
                    "The uploaded image does not sufficiently "
                    "match the trained Ciliates or Diatoms images. "
                ...
                st.write(
                    f"Similarity to known "
                    f"{closest_class} images: "
                    f"{best_similarity:.3f}"
                )
...
# ---------- ANALYSIS ----------
    ...
    st.info(
        "Images that do not sufficiently match "
        "the trained classes are displayed as "
        "Unrecognized for manual verification."
    )
...
# ---------- ABOUT ----------
    ...
    st.write("• Similarity-based unknown screening")
    ...
    st.write(
        ...
        "with an additional similarity-based screening layer "
        "for detecting images that do not sufficiently match "
        "the known training classes."
    )

    st.caption(
        "AquaScope AI — AI + Microscopy for Aquatic "
        "Microorganism Screening"
    )
